...
*What is a step?* A step is anything that you do to you samples. If a step summarizes tasks done over many hours, or if you prefer to create a new step every minute, is entirely decided by yourself. SampleMan creates a labbook entry with time, date and a unique step ID for every step that you enter. You can store templates of step descriptions for steps you perform on a regular basis. Files can be attached to steps, which will be stored in sub-folders of the sample folder, which are named after the step ID. If, for example, there is a cleaning step with the step ID 45 and an attached file "microsocopy.jpg", the folder structure of the database would look like this:
...
Download or clone this repository. Edit `config.json` and set `dbDir` to the desired location of your sample database. Set `uploadDir` to the source directory from which attachment files will be grabbed later. Set `samplePrefix` to the desired prefix of your sample IDs, if it is `ABC` the sample folders will be named `ABC001`, `ABC002`, etc. `stepIdLen` specifies the digit count of the step counter, with a value of 3 up to 1000 steps can be stored and still maintain equivalence of logical and alphabetic order in the folder names.

Then run `sampleman.sh`, wait until all components are installed, and visit `localhost:3000` in your browser. Done.

All settings can of course be changed at any time, but need a restart of `sampleman.sh` to become effective.
